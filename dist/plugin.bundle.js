  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      if (response.status === 403 || response.status === 429) {
        ...
        const waitMs = retryAfter ? parseInt(retryAfter, 10) * 1e3 : (attempt + 1) * 3e3;
      ...
      async execute(toolCallId) {
        try {
          ...
          if (keywords.length === 0) {
            return {
              content: [{ type: "text", text: "\u{1F4CB} \u76D1\u63A7\u5217\u8868\u4E3A\u7A7A\n\n\u4F7F\u7528\u300C\u6DFB\u52A0\u76D1\u63A7\u5173\u952E\u8BCD\u300D\u5F00\u59CB\u8FFD\u8E2A" }],
              details: { count: 0 }
            };
          }
          ...
          return {
            content: [
              {
                type: "text",
                text: `\u{1F4CB} \u5F53\u524D\u76D1\u63A7 ${keywords.length} \u4E2A\u5173\u952E\u8BCD
    ...
    api.registerTool({
      ...
      async execute(toolCallId, params) {
        try {
          ...
          const newAlerts = [...uniqueNew, ...existingAlerts].slice(0, 500);
          await writeJson(getAlertsPath(dataDir), newAlerts);
          const summary = allAlerts.map((a) => `\u2022 **${a.title}**
  \u6765\u6E90: ${a.source} | ${a.keyword}`).join("\n");
          return {
            content: [
              {
                type: "text",
    ...
    api.registerTool({
      ...
      async execute(toolCallId, params) {
        try {
          ...
          if (alerts.length === 0) {
            return {
              content: [{ type: "text", text: "\u{1F4ED} \u6682\u65E0\u544A\u8B66\u8BB0\u5F55\n\n\u5148\u7528\u300C\u5237\u65B0\u76D1\u63A7\u6570\u636E\u300D\u83B7\u53D6\u6700\u65B0\u52A8\u6001" }],
              details: { count: 0 }
            };
          }
          const list = alerts.map((a, i) => `${i + 1}. **${a.keyword}**: ${a.title}
   ${a.url}`).join("\n\n");
          return {
            content: [
              {
                type: "text",
                text: `\u{1F4CA} \u6700\u8FD1 ${alerts.length} \u6761\u544A\u8B66
